...
Calendar.prototype = {
	...
	_init: function(config) {
		 ...
		 * @type String
		 */
		this.id = this.isPopup ? "C_" + (+new Date()) + config.id.replace(/[^0-9]/ig, "") : config.id.replace(/^#/, "") || "C_" + (+new Date());
		/**
		 * 日历容器
		...
		/**
		 * 日期盒子className，默认值"date-box"
		 * @name Calendar.dateBoxClass
		 * @type String
		 */		
		this.dateBoxClass = config.dateBoxClass || "date-box";
		/**
		 * 日期盒子className，默认值"date-input"
		 * @name Calendar.dateInputClass
		 * @type String
		 */		
		this.dateInputClass = config.dateInputClass || "date-input";
		/**
		...
		//自定义 给有价格可选的日期添加亮色
		this.choosable = config.choosable;
		//自定义 当前可选日期第一天
	...
	_create: function() {
		...
		if(this.triggerNode.tagName.toUpperCase() === "INPUT") {
			...
			if(this.isDateInfo) {				
				...
				this.triggerNodeParent.className      = this.dateBoxClass;                 //输入框父类元素样式
				this.triggerNode.className            = this.dateInputClass;			   //输入框元素样式
			...
			for(i = 0, len = arr.length; i < len; i++) {
				if(arr.length) {					
					...
					if(!sValue) {
						...
						if(this.choosable){
							oA.className = 'disabled';
							for(var x = 0; x < this.choosable.length; x++){
								iCur == (_CAL.formatIntDate(this.choosable[x])) && (oA.className = "choosable");
							}
						}else{
						...
						if(this.currentDate){
							iCur == (_CAL.formatIntDate(this.currentDate)) && (oA.className = "selected");
						}
		...
		_CAL.on(obj, "click", function(e) {
			...
			if(oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {					
				...
				if(that.triggerNode.tagName.toUpperCase() === "INPUT") {					
					that.triggerNode.value = oTarget["data-date"]; //oTarget["data-date"] 是 “2014-04-29”
	...
	render: function(date) {
		...
		for(i = 0, len = aCal.length; i < len; i++) {
			month >=0 ? year += (month + (i ? 1 : 0)) > 12 ? 1 : 0 : year -= (month + (i ? 1 : 0)) < 0 ? 1 : 0;
	...
	getDateInfo: function(sDate){
		...
		return {
			...
			holiday: function() {
				...
				//节假日前1-3天/后1-3天日期数
				return that.isHolidayTips ? sWeekName : sWeekName // 只显示星期
				//return that.isHolidayTips ? _dateMap[sDate] || sWeekName : sWeekName  显示节假日前后多少天 - "_dateMap[sDate]"
			}()
		}
	},		
	/**
